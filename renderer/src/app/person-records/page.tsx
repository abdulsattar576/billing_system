...
export default function PersonRecordsPage() {
  ...
  const loadPersonRecords = async (personId: string, areaId: string) => {
    ...
    const personDocs = (res.docs || []).filter(
      (r: any) => r.personId === personId,
    );
    personDocs.sort((a: any, b: any) =>
        ...
        {selectedPerson && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-100 flex flex-wrap gap-6 text-sm">
            ...
            <div>
              ...
              <span
                className={`font-semibold ml-1 ${Number(selectedPerson.remainingBalance || 0) > 0 ? "text-red-600" : "text-green-600"}`}
              >
      ...
      {selectedPerson && filteredRecords.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <p
              className={`text-2xl font-bold ${currentBalance > 0 ? "text-red-600" : "text-green-600"}`}
            >
                  ...
                  {filteredRecords.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                        {r.month
                          ? new Date(r.month + "-01").toLocaleString(
                              "default",
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-gray-800">
                        Rs.
                        {(
                          Number(r.amount || 0) + Number(r.lateFeeCharges || 0)
                        ).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold">
                        <span
                          className={
                            Number(r.remainingAfterPayment || 0) > 0
                              ? "text-red-600"
                              : "text-green-600"
                          }
                        >
      ...
      {!selectedPerson && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-16 text-center">
          ...
          <p className="text-gray-400 text-sm">
            Select an area and a person above to view their full transaction
            history
          </p>
        </div>
      )}
