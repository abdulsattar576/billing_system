...
export default function CashReceivedPage() {
    ...
    try {
      ...
      for (let i = 0; i < months.length; i++) {
        ...
        remainingToAllocate = Math.max(
          0,
          remainingToAllocate - payForThisMonth,
        );
      }
  ...
  const deleteRecord = async (r: any) => {
    ...
    if (
      !confirm(
        `Delete transaction for ${r.personName} (Conn #${r.connectionNumber || "unknown"}) on ${r.month}?`,
      )
    ) {
      return;
    }
  ...
  const expectedPerMonth = Number(selectedPersonFee) || 0;
  const selectedPendingAmount = selectedMonth
    ? Math.max(0, expectedPerMonth - paidInSelectedMonth)
    : 0;

  // all-time: determine person start (createdAt) or earliest payment
  const earliestRecordDate = selectedPersonRecords.reduce(
    (min: string | null, r: any) => {
      if (!r.createdAt) return min;
      const d = new Date(r.createdAt);
      if (!min) return d.toISOString();
      return new Date(min) > d ? d.toISOString() : min;
  ...
  const monthsCount = monthsBetween(startDate, new Date());
  const totalExpectedAllTime = expectedPerMonth * monthsCount;
  const totalPaidAllTime = selectedPersonRecords.reduce(
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600">
          Enter received amounts per person and month
        </p>
      </div>
          ...
          <div>
            ...
            <input
              ...
              value={
                selectedPersonFee === ""
                  ? ""
                  : `${Number(selectedPersonFee).toFixed(2)}`
              }
              readOnly
              className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-black"
            />
          ...
          <div>
            ...
            {(Number(amount) > 0 || Number(lateFeeCharges) > 0) && (
              <div className="mt-1 text-xs text-gray-600">
                Total: Rs.
                {(Number(amount) + Number(lateFeeCharges || 0)).toFixed(2)}
            ...
            {selectedMonth && (
              <div className="p-4 bg-orange-50 rounded-lg border border-orange-100">
                ...
                <div className="text-lg font-semibold text-red-600">
                  ${Number(selectedPendingAmount).toFixed(2)}
                </div>
                <div className="text-sm text-gray-600 mt-1">
                  Expected per month: Rs{Number(expectedPerMonth).toFixed(2)} —
                  Paid: Rs{Number(paidInSelectedMonth).toFixed(2)}
                </div>
              </div>
            )}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-orange-500 uppercase">
                    Late Fee Charges
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    Pending / Balance Due
                    ...
                    <td className="px-6 py-3 text-sm font-medium">
                      {Number(row.lateFeeCharges) > 0 ? (
                        <span className="text-orange-600">
                          Rs.{Number(row.lateFeeCharges).toFixed(2)}
                        </span>
                      ) : (
