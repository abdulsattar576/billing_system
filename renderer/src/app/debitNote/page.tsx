	...
	return (
		<div className="min-h-screen bg-gray-50 p-6">
			<div className="mb-6">
				...
				<p className="text-sm text-gray-600">Enter received amounts per customer</p>
			</div>
			...
			<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end mb-4">
					...
					<div>
						...
						<input type="text" value={selectedPersonAddress} readOnly className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-black" />
					</div>

					<div>
						<label className="block text-sm font-medium text-gray-700 mb-2">Monthly Fee</label>
						<input type="text" value={selectedPersonFee === "" ? "" : `Rs. ${Number(selectedPersonFee).toFixed(2)}`} readOnly className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-black" />
					</div>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
					...
					<div className="flex gap-2">
						<button onClick={addRecord} disabled={saving || !selectedPersonId} className="px-4 py-3 flex-1 bg-gradient-to-r from-blue-600 to-purple-700 text-white text-sm rounded-lg hover:from-blue-700 hover:to-purple-800 transition-colors duration-200 disabled:opacity-50">
							{saving ? "Saving..." : "Save Payment"}
						</button>
						<button onClick={printRecords} disabled={records.length === 0} className="px-4 py-3 flex-1 bg-gradient-to-r from-green-600 to-teal-700 text-white text-sm rounded-lg hover:from-green-700 hover:to-teal-800 transition-colors duration-200 disabled:opacity-50">
							Print
						</button>
					</div>
				</div>
			...
			<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
				<div className="flex justify-between items-center mb-4">
					<h2 className="text-lg font-semibold">
						...
						{selectedPersonId && <span className="text-sm font-normal text-gray-500 ml-2">(for {selectedPersonName})</span>}
					</h2>
					{rowsToShow.length > 0 && !selectedPersonId && (
						<button onClick={printRecords} className="px-4 py-2 bg-green-600 text-white text-sm rounded hover:bg-green-700 transition">
							Print List
						</button>
					)}
				</div>
					...
					<div className="overflow-x-auto">
						<table className="min-w-full divide-y divide-gray-200">
							<thead className="bg-gray-50">
								<tr>
									{!selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Connection #</th>}
									{!selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Customer Name</th>}
									{!selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Area</th>}
									{!selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Receipt No</th>}
									{!selectedPersonId && <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Balance</th>}
									
									{selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date</th>}
									{selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Receipt No</th>}
									{selectedPersonId && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Description</th>}
									{selectedPersonId && <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Amount</th>}
									{selectedPersonId && <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Balance</th>}
									...
									<tr key={row._id} className="hover:bg-gray-50">
										{!selectedPersonId && (
											<>
												...
												<td className="px-6 py-3 text-sm text-gray-500">{getAreaName(row.areaId || selectedArea)}</td>
												<td className="px-6 py-3 text-sm font-medium text-blue-700">{row.receiptNo || '-'}</td>
												<td className={`px-6 py-3 text-sm text-right font-mono ${row.balance < 0 ? "text-red-600" : row.balance > 0 ? "text-green-600" : "text-gray-600"}`}>
													{row.balance < 0 ? `- Rs. ${Math.abs(row.balance).toFixed(2)}` : 
													 row.balance > 0 ? `Rs. ${row.balance.toFixed(2)}` : "0.00"}
													</td>
											</>
										)}
										...
										{selectedPersonId && (
											<>
												<td className="px-6 py-3 text-sm text-gray-600 whitespace-nowrap">
													{row.date ? new Date(row.date).toLocaleDateString() : new Date(row.createdAt).toLocaleDateString()}
													</td>
												<td className="px-6 py-3 text-sm text-gray-600 font-mono text-xs">{row.receiptNo || '-'}</td>
												<td className="px-6 py-3 text-sm text-gray-700">{row.description || '-'}</td>
												<td className={`px-6 py-3 text-sm text-right font-mono ${row.type === "payment" ? "text-green-600" : "text-red-600"}`}>
													{row.type === "payment" ? `+ Rs. ${Number(row.amount).toFixed(2)}` : `- Rs. ${Number(row.amount).toFixed(2)}`}
													</td>
												<td className="px-6 py-3 text-sm text-right font-mono">{formatBalance(row.balance || 0)}</td>
											</>
										)}
										<td className="px-6 py-3 text-sm text-center">
											<button onClick={() => deleteRecord(row)} className="text-red-600 hover:text-red-800 hover:bg-red-50 px-3 py-1 rounded transition-colors duration-200">
												Delete
											</button>
											</td>
									</tr>
