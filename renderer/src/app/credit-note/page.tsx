...
// ─── Schema for a credit note entry ───────────────────────────────────────
// {
//   _id: `credit_note_${areaId}_${personId}_${Date.now()}`,
//   type: "credit-note",
//   areaId: string,
//   personId: string,
...
//   amount: number,           // concession/credit amount
//   description: string,      // reason for credit
//   appliedToDebits: string[], // _id list of customer-debit records reduced
//   createdAt: string,
// }
...
export default function CreditNotePage() {
  ...
  // Customer debit records (unpaid) for this person
  const [debitRecords, setDebitRecords] = useState<any[]>([]);
  // All credit notes for this person
  ...
  // ─── Load debit + credit records ────────────────────────────────────────
  const loadPersonRecords = async (personId: string) => {
    if (!db) return;
    setLoadingRecords(true);
    try {
      const res = await db.localDB.allDocs({ include_docs: true });
      const docs = res.rows.map((r: any) => r.doc).filter((d: any) => d && !d._deleted);

      const debits = docs
        .filter((d: any) => d.type === "customer-debit" && d.personId === personId)
  ...
  // ─── Computed balances ───────────────────────────────────────────────────
  // For each debit, compute how much has been credited against it
  const getEffectiveDebitRows = () => {
    // Sum all credits applied
    const totalCreditGiven = creditNotes.reduce((s, c) => s + Number(c.amount || 0), 0);

    // Distribute credit against oldest unpaid debits first (FIFO)
    let remaining = totalCreditGiven;
    return debitRecords.map((d) => {
      const original = Number(d.amount || 0);
      const alreadyCredited = Math.min(remaining, original);
      remaining = Math.max(0, remaining - original);
      const effectiveAmount = Math.max(0, original - alreadyCredited);
      return { ...d, originalAmount: original, creditedAmount: alreadyCredited, effectiveAmount };
    });
  };

  const effectiveRows = getEffectiveDebitRows();
  const totalDebit = debitRecords.reduce((s, r) => s + Number(r.amount || 0), 0);
  const totalCredited = creditNotes.reduce((s, c) => s + Number(c.amount || 0), 0);
  ...
  const handleSubmit = async () => {
    ...
    if (amount === "" || Number(amount) <= 0) { alert("Please enter a valid credit amount"); return; }
    if (!description.trim()) { alert("Please enter a reason / description for the credit note"); return; }
    ...
    try {
      const doc = {
        _id: `credit_note_${selectedArea}_${selectedPersonId}_${Date.now()}`,
        type: "credit-note",
        areaId: selectedArea,
  ...
  const deleteCredit = async (record: any) => {
    if (!confirm(`Delete credit note of Rs.${record.amount} — "${record.description}"?`)) return;
    try {
  ...
  const printStatement = () => {
    ...
    win.document.write(`
      ...
      <title>Credit Note Statement — ${personName}</title>
      <style>
        body { font-family: Arial, sans-serif; padding: 24px; color: #111; }
        .urdu { text-align:center; direction:rtl; font-size:22px; font-weight:bold; margin-bottom:6px; }
        .sub { text-align:center; font-size:13px; color:#555; margin-bottom:20px; }
        h3 { margin: 20px 0 8px; font-size:14px; color:#374151; border-bottom:1px solid #e5e7eb; padding-bottom:4px; }
        table { width:100%; border-collapse:collapse; margin-bottom:16px; }
        th { background:#f9fafb; padding:9px 10px; text-align:left; font-size:11px; border-bottom:2px solid #e5e7eb; }
        ...
        .net { text-align:right; font-weight:bold; font-size:15px; margin-top:10px; color:#1d4ed8; }
        .summary { display:flex; gap:24px; margin-bottom:16px; }
        .pill { border:1px solid #e5e7eb; border-radius:8px; padding:8px 16px; text-align:center; }
        .pill .label { font-size:10px; color:#6b7280; }
        .pill .val { font-size:14px; font-weight:bold; }
      </style></head><body>
      ...
      <table>
        <thead><tr><th>#</th><th>Date</th><th>Description</th><th>Original</th><th>Credited</th><th>Net Due</th></tr></thead>
        <tbody>
          ${effectiveRows.map((r, i) => `
            <tr>
              ...
              <td>${r.date || "-"}</td>
              <td>${r.description}</td>
              <td>${r.creditedAmount > 0 ? `<span class="strike">Rs.${r.originalAmount.toFixed(2)}</span>` : `Rs.${r.originalAmount.toFixed(2)}`}</td>
              <td class="credit">${r.creditedAmount > 0 ? `Rs.${r.creditedAmount.toFixed(2)}` : "-"}</td>
              <td style="font-weight:bold;color:${r.effectiveAmount > 0 ? "#dc2626" : "#16a34a"}">Rs.${r.effectiveAmount.toFixed(2)}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
      ...
      <table>
        <thead><tr><th>#</th><th>Date</th><th>Reason / Description</th><th>Amount Credited</th></tr></thead>
        <tbody>
          ${creditNotes.map((c, i) => `
            <tr>
              <td>${i + 1}</td>
              <td>${c.date || "-"}</td>
              <td>${c.description}</td>
              <td class="credit">Rs.${Number(c.amount).toFixed(2)}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Credit Note / Concession</h1>
        </div>
        <p className="text-gray-500 text-sm ml-6">
          Issue a concession that reduces the customer's outstanding debit balance
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-7 mb-8">
        ...
        <div className="h-1 w-full bg-gradient-to-r from-green-400 to-emerald-500 rounded-full mb-6"></div>

        <h2 className="text-lg font-semibold text-gray-700 mb-6">Issue Credit Note</h2>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  setAreaSuggestions(
                    areas.filter((a) => a.name.toLowerCase().startsWith(q.toLowerCase())).slice(0, 15)
                  );
                }}
                onKeyDown={(e) => {
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent text-gray-800 placeholder-gray-400"
              />
                ...
                <ul className="absolute z-20 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl max-h-48 overflow-auto shadow-xl">
                  {areaSuggestions.map((a) => (
                    <li
                      ...
                      className="px-4 py-2.5 hover:bg-green-50 cursor-pointer text-sm text-gray-800 border-b border-gray-50 last:border-0"
                    >
                      {a.name}
                    </li>
                  ))}
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  setConnQuery(q);
                  setSelectedPersonId("");
                  setPersonName(""); setPersonAddress(""); setReceiptNo("");
                  setDebitRecords([]); setCreditNotes([]);
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent text-gray-800 placeholder-gray-400 disabled:bg-gray-50 disabled:text-gray-400"
              />
                  ...
                  {connSuggestions.map((p) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {p.receiptNo && (
                          <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full">#{p.receiptNo}</span>
                        )}
                      </div>
                      {p.address && p.address !== "-" && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div>
            ...
            <input type="text" value={personName} readOnly placeholder="Auto-filled"
          ...
          <div>
            ...
            <input type="text" value={personAddress} readOnly placeholder="Auto-filled"
              className="w-full px-4 py-3 border border-gray-200 rounded-xl bg-green-50 text-gray-700 placeholder-gray-400 cursor-default" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Receipt No</label>
            <input type="text" value={receiptNo} readOnly placeholder="Auto-filled"
              className="w-full px-4 py-3 border border-gray-200 rounded-xl bg-green-50 text-gray-700 placeholder-gray-400 cursor-default" />
          </div>
        </div>

        {/* Net balance indicator — shown when person selected */}
        {selectedPersonId && (
          <div className="flex flex-wrap gap-4 mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
            <div className="text-center">
              ...
              <div className="text-base font-bold text-red-600">Rs.{totalDebit.toFixed(2)}</div>
            </div>
            <div className="flex items-center text-gray-300 font-light text-xl">−</div>
            <div className="text-center">
              <div className="text-xs text-gray-400 mb-0.5">Already Credited</div>
              <div className="text-base font-bold text-green-600">Rs.{totalCredited.toFixed(2)}</div>
            </div>
            <div className="flex items-center text-gray-300 font-light text-xl">=</div>
            <div className="text-center">
              <div className="text-xs text-gray-400 mb-0.5">Net Balance</div>
              <div className={`text-base font-bold ${netBalance > 0 ? "text-blue-700" : "text-green-600"}`}>
            ...
            {Number(amount) > 0 && (
              <>
                <div className="flex items-center text-gray-300 font-light text-xl">→</div>
                <div className="text-center">
                  <div className="text-xs text-gray-400 mb-0.5">After This Credit</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Damaged cable concession, loyalty discount, return of faulty item..."
              rows={1}
        ...
        <div className="flex gap-4 items-center flex-wrap">
          <button
            ...
            className="px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-700 text-white rounded-xl hover:from-green-700 hover:to-emerald-800 transition-all duration-200 font-semibold disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
            {submitting ? "Saving..." : "Issue Credit Note"}
          </button>

          {selectedPersonId && (debitRecords.length > 0 || creditNotes.length > 0) && (
            <button
              onClick={printStatement}
              className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-xl hover:from-blue-700 hover:to-indigo-800 transition-all duration-200 font-semibold shadow-sm"
            >
              Print Statement
            </button>
          )}
      ...
      {selectedPersonId && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          {/* Left: Debit records with credit applied */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-red-50">
              <h2 className="text-base font-semibold text-gray-800">
                Customer Debits
                <span className="ml-2 text-xs font-normal text-gray-500">(credit applied oldest first)</span>
              </h2>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      {["Date", "Description", "Original", "Credited", "Net Due"].map((h) => (
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {effectiveRows.map((row) => (
                      <tr key={row._id} className={`hover:bg-gray-50 transition-colors ${row.effectiveAmount === 0 ? "opacity-50" : ""}`}>
                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
                          {row.date ? new Date(row.date).toLocaleDateString("en-PK", { day: "numeric", month: "short", year: "numeric" }) : "-"}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-800 max-w-[160px] truncate" title={row.description}>
                          {row.description}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">
                          {row.creditedAmount > 0
                            ? <span className="line-through text-gray-400">Rs.{row.originalAmount.toFixed(2)}</span>
                            : <span>Rs.{row.originalAmount.toFixed(2)}</span>}
                        </td>
                        <td className="px-4 py-3 text-sm whitespace-nowrap">
                          {row.creditedAmount > 0
                            ? <span className="text-green-600 font-semibold">− Rs.{row.creditedAmount.toFixed(2)}</span>
                            : <span className="text-gray-300">—</span>}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`text-sm font-bold ${row.effectiveAmount === 0 ? "text-green-600" : "text-red-600"}`}>
                  ...
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                    <tr>
                      <td colSpan={2} className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase">Total</td>
                      <td className="px-4 py-3 text-sm font-bold text-red-600">Rs.{totalDebit.toFixed(2)}</td>
                      <td className="px-4 py-3 text-sm font-bold text-green-600">− Rs.{totalCredited.toFixed(2)}</td>
                      <td className="px-4 py-3 text-sm font-bold text-blue-700">Rs.{netBalance.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
          ...
          {/* Right: Credit notes issued */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-green-50">
              <h2 className="text-base font-semibold text-gray-800">Credit Notes Issued</h2>
            </div>
            ...
            {loadingRecords ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-gray-300 text-xs mt-1">Issue the first credit note above</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      {["Date", "Reason / Description", "Amount", "Actions"].map((h) => (
                        <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {creditNotes.map((cn) => (
                      <tr key={cn._id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
                          {cn.date ? new Date(cn.date).toLocaleDateString("en-PK", { day: "numeric", month: "short", year: "numeric" }) : "-"}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-800 max-w-[180px]">
                          <div className="font-medium truncate" title={cn.description}>{cn.description}</div>
                  ...
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                    <tr>
                      <td colSpan={2} className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase">Total Credited</td>
                      <td className="px-4 py-3 text-sm font-bold text-green-600">− Rs.{totalCredited.toFixed(2)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
